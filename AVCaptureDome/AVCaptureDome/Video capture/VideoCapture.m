...
+ (VideoCapture *)captureWith:(UIView *)preView{
    ...
    if (!cap.captureSession) {
        //AVCaptureSession 配置失败重新配置
        if (![cap setupSession:[NSError new]]) {
            cap.captureSession = nil;
        }
    }
    [cap.PreView removeFromSuperview];
    cap.PreView.frame = preView.bounds;
    [cap.PreView setSession:cap.captureSession];
    [preView addSubview:cap.PreView];
    [cap startSession];
    return cap;
}
...
- (void)resetFocusAndExposureModes {
    ...
    ///用于判断设备是否支持设置区域焦距调节的功能和设备能否实现对自动变焦到合适的焦距（手机前置摄像头是没有焦距调节的）
...
- (void)writeImageToAssetsLibrary:(UIImage *)image {
    if (self.delegate && [self.delegate respondsToSelector:@selector(captureImage:)]) {
    ...
    if (status == PHAuthorizationStatusRestricted ||
        ...
        if ((self.delegate && [self.delegate respondsToSelector :@selector(captureFailedWithError:)])) {
            [self.delegate captureFailedWithError:[NSError errorWithDomain:@"fail" code:-10002 userInfo:@{@"msg":@"没有相机权限"}]];
        }
    } else {
...
- (NSURL *)uniqueURL {
    ...
    NSString *temp = @"avcapture";
    NSString *mkdTemplate =
    [NSTemporaryDirectory() stringByAppendingPathComponent:temp];
    if (![fileManager fileExistsAtPath:mkdTemplate]) {
        [fileManager createDirectoryAtPath:mkdTemplate withIntermediateDirectories:YES attributes:nil error:nil];
    }
    NSString *filePath  = [mkdTemplate stringByAppendingPathComponent:@"video.mov"];
...
- (void)writeVideoToAssetsLibrary:(NSURL *)videoURL {
    ...
    [self exporterDidEnd:[videoURL path]];
...
- (void)tappedToFocusAtPoint:(CGPoint)point {
    [self focusAtPoint:point]; //对焦
}

- (void)tappedToExposeAtPoint:(CGPoint)point {
    [self exposeAtPoint:point]; //曝光
}

- (void)tappedToResetFocusAndExposure {
    [self resetFocusAndExposureModes]; //从新对焦和曝光
}

- (void)setTapToFocusEnabled:(BOOL)enabled {
    [self.PreView setTapToFocusEnabled:enabled];
}

- (void)setTapToExposeEnabled:(BOOL)enabled {
    [self.PreView setTapToExposeEnabled:enabled];
}

- (CapturePreView *)PreView{
    if (!_PreView) {
        _PreView = [[CapturePreView alloc]init];
...
- (void)exporterDidEnd:(NSString *)outputPath {
    if (self.delegate && [self.delegate respondsToSelector:@selector(captureVideo:)]) {
        [self.delegate captureVideo:outputPath];
    }
    if (!self.saveVideoToAlbum) {
           return;
       }
    ...
    if (status == PHAuthorizationStatusRestricted ||
        ...
        [PHPhotoLibrary requestAuthorization:^(PHAuthorizationStatus status) {
            if (status == PHAuthorizationStatusAuthorized) {
                dispatch_async(dispatch_get_main_queue(), ^{
                    [self saveVideoToAlbum:outputPath];
                });
            }
        }];
    }
...
- (void)updateTimeDisplay{
    NSLog(@"%d",(NSUInteger)CMTimeGetSeconds([self recordedDuration]));
    if (self.delegate && [self.delegate respondsToSelector:@selector(updateCaptureTimeDisplay:)]) {
        [self.delegate updateCaptureTimeDisplay:[self recordedDuration]];
    }
}
- (void)creatImage:(UIImage *)image{
    if (self.delegate && [self.delegate respondsToSelector:@selector(creatImage:)]) {
        [self.delegate creatImage:image];
    }
}
